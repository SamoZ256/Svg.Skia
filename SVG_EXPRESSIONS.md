...
`--singleFile` writes the whole batch into one C# file instead of one file per item. The items'
`OutputFile` is then ignored, so the same batch works either way:

```sh
svgc -j icons.json -r icons.recipe -n Icons --singleFile Icons.cs
```

Classes keep their declared order and namespaces the order they first appear in, so the output of
a given batch does not move between runs. Two drawings of one class name in one namespace is an
