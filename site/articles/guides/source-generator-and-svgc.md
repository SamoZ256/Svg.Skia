...
`src/Svg.Studio` builds the same projects, from a format of its own that holds the drawings rather
than pointing at them — and opens one of these by converting it. See
...
- [Svg Studio](svg-studio) — the editor, and the one-file project format it works in
