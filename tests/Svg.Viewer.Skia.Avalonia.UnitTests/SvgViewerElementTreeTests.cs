...
public class SvgViewerElementTreeTests
{
    ...
    /// <summary>The attribute panel, which is a tab's content rather than a realised visual.</summary>
    ...
    [AvaloniaFact]
    public async Task Several_Rows_Can_Be_Picked_At_Once()
    {
        var (_, viewer) = await Host();
        ...
        Assert.Equal(new[] { "rect", "text" }, viewer.Elements.SelectedNodes.Select(node => node.Label));

        // Document order, whatever order they were picked in, because that is the order they are
        // written in and the order anything editing them has to work in.
        Assert.True(viewer.Elements.TrySelect(new[] { "1/1", "1/0" }));
        Dispatcher.UIThread.RunJobs();
    ...
    /// <summary>
    /// A rebuild throws every row away, and what was picked has to come back.
    /// </summary>
    /// <remarks>
    /// Held by address, exactly as the open branches are: the rebuilt drawing shares no element with
    /// the one it replaced, so a selection held by element would be gone on the next keystroke.
    /// </remarks>
    [AvaloniaFact]
    public async Task A_Selection_Of_Several_Survives_A_Rebuild()
    {
        ...
        var was = viewer.Elements.SelectedNodes[0].Element;

        Assert.True(viewer.Rebuild());
        Dispatcher.UIThread.RunJobs();

        Assert.Equal(new[] { "rect", "text" }, viewer.Elements.SelectedNodes.Select(node => node.Label));
        Assert.NotSame(was, viewer.Elements.SelectedNodes[0].Element);
    }

    [AvaloniaFact]
    public async Task A_Picked_Row_That_Has_Gone_Drops_Out_Of_The_Selection()
    {
        var (_, viewer) = await Host();
        ...
        Assert.True(await viewer.LoadTextAsync(Markup.Replace("""<text x="2" y="20">hi</text>""", string.Empty)));
        Dispatcher.UIThread.RunJobs();

        // One left, not none: what is still there is still picked.
        Assert.Equal(new[] { "rect" }, viewer.Elements.SelectedNodes.Select(node => node.Label));
    }

    [AvaloniaFact]
    public async Task Picking_One_Row_Puts_The_Others_Down()
    {
        var (_, viewer) = await Host();
        ...
        Assert.Equal(new[] { "text" }, viewer.Elements.SelectedNodes.Select(node => node.Label));
    }

    [AvaloniaFact]
    public async Task The_Ring_Covers_Everything_Picked()
    {
        var (_, viewer) = await Host();
        ...
        // The text sits below the rect, so ringing both has to reach further down than either.
        var both = viewer.Canvas.Highlight!.Bounds;

        Assert.True(both.Bottom >= one.Bottom, "the ring did not cover the second row");
        Assert.True(both.Contains(one), "the ring stopped covering the first row");
    }

    /// <summary>One panel writes one element, so it says so rather than writing the first.</summary>
    [AvaloniaFact]
    public async Task The_Attribute_Panel_Steps_Aside_For_Several()
    {
        var (_, viewer) = await Host();
        ...
        Assert.False(string.IsNullOrEmpty(Panel(viewer).Shown("fill")));
