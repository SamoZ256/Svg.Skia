...
public sealed class PaintCodeImportOptions
{
    ...
    /// </summary>
    /// <remarks>
    /// Off, and a guess when it is on. PaintCode has one numeric kind: its type tags are number,
    /// string, boolean, colour, gradient and rect, and a value is archived as a real whether or not
    /// it is whole -- a width of 15 and an alpha of 1 are both reals in the only document committed
    /// here. Its bounds are a min and a max with no step, so nothing in the format distinguishes a
    /// step enum from a slider that happens to sit on a whole number.
    ///
    ...
    /// float. Turning it on retypes those, and will also retype a continuous slider whose ends and
    /// value are whole -- which is why the author asks for it rather than being given it.
    /// </remarks>
