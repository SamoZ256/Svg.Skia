...
public static class SkiaCSharpCodeGen
{
    ...
    public static string Generate(SKPicture picture, string namespaceName, string className, SvgCodeDeclarations? declarations)
    {
        ...
        // The usings sit outside the namespace, not inside it. A using written inside a
        // namespace resolves relative to it first, so `using SkiaSharp;` inside `namespace Foo`
        // binds to `Foo.SkiaSharp` when the consumer happens to have one — and the generated
        // file then fails to compile, or quietly resolves the wrong types.
