...
- **Resizes the drawing**, by rewriting the `width`, `height` and `viewBox` its root element
  declares — the same arithmetic svgc resizes by, written back as a text edit, so the pane shows it
  and an undo takes it back.
